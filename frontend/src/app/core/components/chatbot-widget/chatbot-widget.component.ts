...
export class ChatbotWidgetComponent implements OnInit, OnDestroy {
  ...
  cursosVirtuales = [
    ...
    { emoji: '👂', name: 'Seminario Auriculoterapia', route: '/cursos/control-peso-auriculoterapia-acupuntura-online' },
  ...
  getCurrentCourseName(): string | null {
    ...
    if (path.includes('sedes')) return 'las Sedes y Horarios de INSTEIP';
  ...
  getWhatsAppUrl(specificCourse?: string): string {
    ...
    let message = 'Hola Insteip, quiero información sobre cómo inscribirme.';
    if (course) {
      if (course.startsWith('el ') || course.startsWith('la ') || course.startsWith('los ') || course.startsWith('las ')) {
        message = `Hola Insteip, quiero información sobre cómo inscribirme en ${course}`;
      } else {
        message = `Hola Insteip, quiero información sobre cómo inscribirme en el curso de ${course}`;
      }
    }
  ...
  loadChatHistory(): void {
    if (typeof window !== 'undefined' && window.sessionStorage) {
      const historyJson = sessionStorage.getItem('insteip-chatbot-history');
      if (historyJson) {
        try {
  ...
  saveChatHistory(): void {
    if (typeof window !== 'undefined' && window.sessionStorage) {
      sessionStorage.setItem('insteip-chatbot-history', JSON.stringify(this.messages));
    }
  }
  ...
  sendMessage(): void {
    ...
    if (
      ...
      setTimeout(() => {
        this.messages.push({
          role: 'assistant',
          content: '🌿 **Sede Huánuco - Coordinación Oficial**\n\nPrácticas clínicas presenciales y eventos intensivos.\n\n[button:💬 Contactar vía WhatsApp](https://wa.me/51935354183?text=Hola%20Insteip%2C%20deseo%20contactar%20con%20la%20Coordinaci%C3%B3n%20de%20la%20Sede%20Hu%C3%A1nuco%20para%20m%C3%A1s%20informaci%C3%B3n)\n[button:📍 Conocer Nuestras Sedes](/sedes)'
        });
        this.isLoading = false;
        this.suggestions = ['🏫 Cursos Presenciales', '💻 Cursos Online', '🔄 Volver al Inicio'];
        this.saveChatHistory();
