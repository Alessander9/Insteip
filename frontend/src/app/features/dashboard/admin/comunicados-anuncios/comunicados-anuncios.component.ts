...
export class ComunicadosAnunciosComponent implements OnInit, OnDestroy {
  ...
  whatsappMsgAnuncio = '¡Hola INSTEIP! Deseo más información sobre la promoción.';
  ...
  whatsappMsgComunicado = '¡Hola INSTEIP! Tengo una consulta sobre el comunicado.';
  ...
  descargarAdjunto(item: { adjuntoUrl?: string; adjuntoNombre?: string }): void {
    ...
    a.download = item.adjuntoNombre || 'documento_insteip';
  ...
  aplicarPlantilla(tipo: 'seminario' | 'bienvenida' | 'recordatorio_tareas' | 'certificados', destino: 'anuncio' | 'comunicado'): void {
    if (tipo === 'seminario') {
      ...
      if (destino === 'anuncio') {
        ...
        this.whatsappMsgAnuncio = 'Hola INSTEIP, deseo inscribirme al Seminario Internacional.';
        this.actualizarLinkAnuncio();
      } else {
      ...
      const titulo = '🎓 ¡Bienvenidos al Nuevo Ciclo Académico INSTEIP! 🚀';
