...
export class MensajesComponent implements OnInit, OnDestroy {
  ...
  // Auto-draft keys
  private readonly DRAFT_NEW_KEY = 'insteip_draft_new_message';
  ...
  guardarBorradorRespuesta(): void {
    if (this.selectedConversacion) {
      localStorage.setItem(`insteip_draft_reply_${this.selectedConversacion.id}`, this.respuestaTexto);
    }
  }

  private cargarBorradorRespuesta(convId: number): void {
    const saved = localStorage.getItem(`insteip_draft_reply_${convId}`);
    this.respuestaTexto = saved || '';
  }

  private limpiarBorradorRespuesta(convId: number): void {
    localStorage.removeItem(`insteip_draft_reply_${convId}`);
  }
