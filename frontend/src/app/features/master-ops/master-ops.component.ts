  ...
  private parseLabeledBlocks(text: string): ParsedStudent[] {
    ...
    const pushCurrentIfValid = () => {
      if (current.nombres || current.correo) {
        ...
        const correo = current.correo || (n ? `${n.toLowerCase().replace(/\s+/g, '')}@insteip.com` : '');
    ...
    for (const line of lines) {
      ...
      if (parts.length === 1) {
        ...
        if (emailIdx > -1) {
          ...
          apellidos = parts[1];
          telefono = parts[2] || '';
          correo = `${nombres.toLowerCase().replace(/\s+/g, '')}.${apellidos.toLowerCase().replace(/\s+/g, '')}@insteip.com`;
        }
      }
