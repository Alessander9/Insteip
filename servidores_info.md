...
| **Carpeta de Assets (Imágenes)** | `/var/www/insteip/assets/` |
| **Archivo Ejecutable Backend** | `/opt/insteip/app/backend.jar` |
| **Archivos Subidos / Materiales** | `/opt/insteip/data/materiales/` |
| **Configuración de Variables** | `/etc/insteip/backend.env` |
| **Configuración Nginx (activa)** | `/etc/nginx/sites-available/insteip` ← **ESTE es el archivo activo** |
| **Enlace simbólico Nginx** | `/etc/nginx/sites-enabled/insteip` |

> ⚠️ **IMPORTANTE:** El archivo de configuración de Nginx que está realmente activo para `insteip.com` es `/etc/nginx/sites-available/insteip`, **NO** el `default`. El archivo `default` está en el servidor pero no está habilitado para este dominio.
...
scp -r -i "C:\Users\Alessander\.ssh\id_ed25519" frontend/dist/frontend/* root@62.146.226.81:/var/www/insteip/
...
ssh -i "C:\Users\Alessander\.ssh\id_ed25519" root@62.146.226.81 "chmod 755 /var/www/insteip/assets && find /var/www/insteip/assets -type f -exec chmod 644 {} \; && find /var/www/insteip/assets -type d -exec chmod 755 {} \;"
...
cd ..
scp -i "C:\Users\Alessander\.ssh\id_ed25519" backend/target/*.jar root@62.146.226.81:/opt/insteip/app/backend.jar
ssh -i "C:\Users\Alessander\.ssh\id_ed25519" root@62.146.226.81 "systemctl restart insteip-backend"
```
...
**Causa:** El directorio `/var/www/insteip/assets/` se sube con permisos `drwx------` (solo accesible por el usuario `insteip`). Nginx corre como `www-data` y no puede leer los archivos.

**Solución:** Ejecutar SIEMPRE después de cada SCP el paso 3 del despliegue:
```bash
chmod 755 /var/www/insteip/assets
find /var/www/insteip/assets -type f -exec chmod 644 {} \;
find /var/www/insteip/assets -type d -exec chmod 755 {} \;
```
...
### Error: Nginx no carga el sitio de insteip.com al editar configuración
**Causa:** Editar el archivo `/etc/nginx/sites-available/default` en lugar del archivo correcto.

**Solución:** El archivo de configuración activo es `/etc/nginx/sites-available/insteip`. Para editar:
```bash
...
- **Verificar que una imagen es accesible (reemplazar nombre):**
  ```bash
  curl -s -o /dev/null -w '%{http_code}' https://insteip.com/assets/insteip-logo.png
  # Debe devolver 200. Si devuelve 403, ejecutar el fix de permisos del paso 3.
  ```
- **Ver permisos del directorio assets:**
  ```bash
  ls -la /var/www/insteip/assets/ | head -5
